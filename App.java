...
//Imports de tudo que vai ser usado
...
public class App {
    ...
    static int n = 0; //numero de tarefas

    public static void main(String[] args) throws Exception {
        //carrga as tarefas do CSV
        carregar();
        //cria um servidor pra acessar o codigo depois
        HttpServer server = HttpServer.create(new InetSocketAddress(PORT), 0);
        //define o caminho que vai ser usado
        server.createContext("/", new RootHandler()); //pagina inicial HTML
        server.createContext("/api/tasks", new ApiTasksHandler()); // API das tarefas
        server.setExecutor(null);
        System.out.println("Servindo em http://localhost:" + PORT);
        server.start();
    }
    //Caminho que vai ser utilizado pela pagina inicial HTML
    ...
    static class ApiTasksHandler implements HttpHandler {
        @Override public void handle(HttpExchange ex) throws IOException {
            ...
            try {
                ...
                //criar novas tarefas com titulo e descrição
                if ("POST".equals(method) && "/api/tasks".equals(path)) {
                    ...
                    //verifica se a tarefa tem titulo
                ...
                //deleta uma tarefa criada anteriormente
                if ("DELETE".equals(method) && path.startsWith("/api/tasks/")) {
                    String id = path.substring("/api/tasks/".length());
                    int i = findIdxById(id);
                    //verifica se a tarefa é real e retorna erro caso não seja
                    if (i < 0) { sendJson(ex, 404, "{\"error\":\"not found\"}"); return; }
                    //remove o numero da tarefa e passa para outras para preencher o vazio
                    for (int k = i; k < n - 1; k++) {
                ...
                //caso nenhuma informação acima seja verdadeira retorna erro 404
                send(ex, 404, "");
            } catch (Exception e) {
                e.printStackTrace();
    ...
    //pagina inicial do Html, CSS, e JavaScript
...
""";
    ...
    //le o CSV e leva as tarefas para o armazenamentro
    ...
    //encontra o indice da tarefa pelo id
    static int findIdxById(String id){
        for (int i=0;i<n;i++) if (ids[i].equals(id)) return i;
        return -1;
    }
    //cria um mapa para representar a tabela das tarefas
    ...
    //todas as funçoes abaixo servem para mudar/manipular as Strings, numeros e JSONs
